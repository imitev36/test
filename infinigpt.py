...
class ircGPT(irc.bot.SingleServerIRCBot):
    def __init__(self, personality, channels, nickname, server, password=None, port=6667):
    ...
    def respond(self, c, sender, message, sender2=None):
        try:
            ...
            #if .x function used
            if sender2:
                c.privmsg(self.channels, sender2 + ":")
            #normal .ai usage
            else:
                c.privmsg(self.channels, sender + ":")
            time.sleep(1)
    ...
    def on_welcome(self, c, e):
        ...
        c.send_raw("NAMES " + ' '.join(self.channels))
        ...
        try:
            ...
            response_text = response['choices'][0]['message']['content']
            c.privmsg(self.channels, response_text + f"  Type .help {self.nickname} to learn how to use me.")
        except:
            pass
    ...
    def on_pubmsg(self, c, e):
        ...
        if sender != self.nickname:
            ...
            if message.startswith(".ai") or message.startswith(self.nickname):
                ...
                if flagged:
                    c.privmsg(self.channels, f"{sender}: This message violates OpenAI terms of use and was not sent")
            ...
            if message.startswith(".x "):
                ...
                if len(m) > 1:
                    #get users in channel
                    c.send_raw("NAMES " + self.channels)
            ...
            if message.startswith(".persona "):
                ...
                flagged = self.moderate(m) #set to False if you want to bypass moderation
                if flagged:
                    c.privmsg(self.channels, f"{sender}: This persona violates OpenAI terms of use and was not set.")
                    #add way to ignore user after a certain number of violations
                else:
            ...
            if message.startswith(".custom "):
                ...
                flagged = self.moderate(m) #set to False if you want to bypass moderation
                if flagged:
                    c.privmsg(self.channels, f"{sender}: This custom prompt violates OpenAI terms of use and was not set.")
                    #add way to ignore user after a certain number of violations
                else:
            ...
            if message.startswith(".reset"):
                self.reset(sender)
                c.privmsg(self.channels, f"{self.nickname} reset to default for {sender}.")
            ...
            if message.startswith(".stock"):
                if sender in self.messages:
                    ...
                    self.messages[sender] = []                    
                c.privmsg(self.channels, f"Stock settings applied for {sender}")
...
#                self.channel.append(channel)
                c.privmsg(self.channels, f"{self.nickname} join to {new_chan}")
...
#                self.channel.append(channel)
                c.privmsg(self.channels, f"{self.nickname} part {target_channel}")
...
if __name__ == "__main__":
    ...
    try:
      infiniGPT = ircGPT(personality, channels, nickname, server, password)
    except:
      infiniGPT = ircGPT(personality, channels, nickname, server)
