    ...
    def connect(self):        
        ...
        try:
            self.connection = self.reactor.server().connect(
                ...
                REALNAME  # Pass the REALNAME here
            )
        ...
        self.connection.add_global_handler("privmsg", self.privmsg)
	    ...
	    # Handle goaway command
            if input_text.startswith("!join"):
                if sender_username in ADMIN_NICKNAMES:
                    ...
                    try:
                   self.connection.privmsg(self.channel, MSG_GOAWAY_SUCCESS)
                     channel = input_text.split(" ")[1]  # Vzemame imeto na kanala
                     self.part_channel(channel)  # Napuskame kanala
                else:
                    self.connection.privmsg(self.channel, MSG_NO_GOAWAY_PRIV.format(sender_username))
